...
#
#   DOMAIN_SETUP   Set to 'wlst' to setup a new domain via wlst, or 'WDT'
#                  to setup via the weblogic-deploy-tool.
#                  Default is 'WDT'.
#
...
function run_create_domain_job {
    ...
    # When setting up a domain via WDT instead of wlst, deploy a specially
    # named config map with the WDT launch script, model, and property file.  The
    # 'create weblogic domain job' will detect this map and use WDT instead of wlst.
    # (Uses the 'create wl domain job inputs' yaml file as the model's property file.)

    if [ "$DOMAIN_SETUP" = "WDT" ]; then
       trace "WDT: Configuring cluster via WDT since this is a configured cluster."

       local wdt_dir=$tmp_dir/wdt
       local wdt_configmap="${DOMAIN_UID}-create-weblogic-domain-job-wdt-cm"

       if [ "$WL_CLUSTER_TYPE" = "CONFIGURED" ]; then
         local wdt_model_source="$PROJECT_ROOT/wdt/wdt_model_configured.yaml"
       else
         local wdt_model_source="$PROJECT_ROOT/wdt/wdt_model_dynamic.yaml"
       fi

       mkdir $wdt_dir \
         || fail "WDT - Failed to mkdir $wdt_dir for staging WDT files."

       cp $inputs $wdt_dir \
         || fail "WDT - Failed to copy $inputs to $wdt_dir"

       cp $PROJECT_ROOT/wdt/createDomain.sh $wdt_dir \
         || fail "WDT - Failed to copy $PROJECT_ROOT/wdt/createDomain.sh to $wdt_dir"

       cp $wdt_model_source $wdt_dir/wdt_model.yaml \
         || fail "WDT - Failed to copy $wdt_model_source to $wdt_dir/wdt_model.yaml"

       kubectl -n ${NAMESPACE} create configmap ${wdt_configmap} --from-file=${wdt_dir} \
         || fail "WDT - Failed to deploy configmap"

       kubectl -n ${NAMESPACE} label configmap ${wdt_configmap} domainUID=${DOMAIN_UID} \
         || fail "WDT - Failed to label wdt configmap"

       trace "WDT - Deployed configmap $wdt_configmap with files (`ls $wdt_dir | xargs echo`).  The create-weblogic-domain.sh kubernetes job will detect the configmap and run WDT instead of wlst."
    fi
