...
class ProcessedChart {
  ...
  private List<Map<String, String>> getDocuments() throws Exception {
    Iterable<Object> it = new Yaml().loadAll(getProcess().getInputStream());
    List<Map<String, String>> charts = new ArrayList<>();
    it.forEach(
        (o) -> {
          if (o != null) charts.add((Map<String, String>) o);
        });
  ...
  private File getChartsDir(String chartName) throws URISyntaxException {
    return new File(getSrcDir(getClass()).getParentFile(), "kit/charts/" + chartName);
  }

  private File getSrcDir(Class<?> aClass) throws URISyntaxException {
    File dir = getPackageDir(aClass);
    while (dir.getParent() != null && !dir.getName().equals("src")) {
