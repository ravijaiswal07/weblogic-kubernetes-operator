...
function createYamlFiles {
  ...
  if [ "${loadBalancer}" = "VOYAGER" ] || [ "${generateHelm}" = true ] ; then
    ...
    sed -i -e "s:%IF_LOADBALANCER_VOYAGER%:${helmIfLoadbalancerVoyager}:g" ${voyagerIngressOutput}
    sed -i -e "s:%END_IF%:${helmEndIf}:g" ${voyagerIngressOutput}
    sed -i -e "s:%IF_LOADBALANCER_VOYAGER%:${helmIfLoadbalancerVoyager}:g" ${voyagerSecurityOutput}
    sed -i -e "s:%END_IF%:${helmEndIf}:g" ${voyagerSecurityOutput}
    sed -i -e "s:%IF_LOADBALANCER_VOYAGER%:${helmIfLoadbalancerVoyager}:g" ${voyagerOperatorOutput}
    sed -i -e "s:%END_IF%:${helmEndIf}:g" ${voyagerOperatorOutput}
  fi
