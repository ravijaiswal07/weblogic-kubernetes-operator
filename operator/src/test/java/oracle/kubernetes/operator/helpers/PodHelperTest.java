...
public class PodHelperTest {
  ...
  private static final V1VolumeMount EXPECTED_WEBLOGIC_DOMAIN_STORAGE_VOLUME_MOUNT =
      newVolumeMount().name("weblogic-domain-storage-volume").mountPath("/shared");

  private static final V1VolumeMount EXPECTED_WEBLOGIC_CREDENTIALS_VOLUME_MOUNT =
      newVolumeMount()
          .name("weblogic-credentials-volume")
          .mountPath("/weblogic-operator/secrets")
          .readOnly(true);

  private static final V1VolumeMount EXPECTED_WEBLOGIC_CONFIG_MAP_VOLUME_MOUNT =
      newVolumeMount()
          .name("weblogic-domain-cm-volume")
          .mountPath("/weblogic-operator/scripts")
          .readOnly(true);

  private static final V1Volume EXPECTED_WEBLOGIC_DOMAIN_STORAGE_VOLUME =
      newVolume()
  ...
  private static final V1Volume EXPECTED_WEBLOGIC_CREDENTIALS_VOLUME =
      newVolume()
          .name("weblogic-credentials-volume")
          .secret(newSecretVolumeSource().secretName(WEBLOGIC_CREDENTIALS_SECRET_NAME));

  private static final V1Volume EXPECTED_WEBLOGIC_CONFIG_MAP_VOLUME =
      newVolume()
          .name("weblogic-domain-cm-volume")
  ...
  */
  ...
  public void setWeblogicServerImage_addsCorrectResources() {
    ...
    V1PodSpec podSpecWant = newPodSpec().imagePullSecrets(serverConfig.getImagePullSecrets());
    V1Container containerWant =
        newContainer()
            .image(serverConfig.getImage())
            .imagePullPolicy(serverConfig.getImagePullPolicy());
  ...
  @Test
  public void addVolumes_oneClaim_addsCorrectVolumesAndMounts() {
    addVolumes_addsCorrectVolumesAndMounts(
  ...
  @Test
  public void addVolumes_noClaims_addsCorrectVolumesAndMounts() {
    addVolumes_addsCorrectVolumesAndMounts(newPersistentVolumeClaimList());
  }

  private void addVolumes_addsCorrectVolumesAndMounts(V1PersistentVolumeClaimList claims) {
    V1PodSpec actualPodSpec = newPodSpec();
    V1Container actualContainer = newContainer();
    PodHelper.addVolumes(actualPodSpec, actualContainer, DOMAIN_SPEC, claims);

    V1PodSpec podSpecWant = newPodSpec();
    boolean haveClaim = !(claims.getItems().isEmpty());
    addExpectedVolumes(podSpecWant, haveClaim);
    V1Container containerWant = newContainer();
    addExpectedVolumeMounts(containerWant);
  ...
  private void addExpectedVolumeMounts(V1Container container) {
    container
        .addVolumeMountsItem(EXPECTED_WEBLOGIC_DOMAIN_STORAGE_VOLUME_MOUNT)
        .addVolumeMountsItem(EXPECTED_WEBLOGIC_CREDENTIALS_VOLUME_MOUNT)
        .addVolumeMountsItem(EXPECTED_WEBLOGIC_CONFIG_MAP_VOLUME_MOUNT);
  }

  private void addExpectedVolumes(V1PodSpec podSpec, boolean haveClaim) {
    if (haveClaim) {
      podSpec.addVolumesItem(EXPECTED_WEBLOGIC_DOMAIN_STORAGE_VOLUME);
    }
    podSpec
        .addVolumesItem(EXPECTED_WEBLOGIC_CREDENTIALS_VOLUME)
        .addVolumesItem(EXPECTED_WEBLOGIC_CONFIG_MAP_VOLUME);
  }
        ...
        newPod()
            .metadata(
                getExpectedServerPodMetadata(
                    MANAGED_SERVER_NAME, MANAGED_SERVER_PORT, RESTARTED_LABEL1));

    assertThat(actual, equalTo(want));
  }

  private V1ObjectMeta getExpectedServerPodMetadata(
      String serverName, int port, String restartedLabel) {
    V1ObjectMeta metadata =
        newObjectMeta()
            .name(DOMAIN_UID + "-" + serverName.toLowerCase())
            .namespace(NAMESPACE)
            .putAnnotationsItem("prometheus.io/path", "/wls-exporter/metrics")
            .putAnnotationsItem("prometheus.io/port", "" + MANAGED_SERVER_PORT)
            .putAnnotationsItem("prometheus.io/scrape", "true")
            .putLabelsItem(RESOURCE_VERSION_LABEL, DOMAIN_V1)
            .putLabelsItem(CREATEDBYOPERATOR_LABEL, "true")
            .putLabelsItem(DOMAINNAME_LABEL, DOMAIN_NAME)
    ...
    if (restartedLabel != null) {
      metadata.putLabelsItem(RESTARTED_LABEL, RESTARTED_LABEL1);
    }
    return metadata;
  }
  ...
  public void addRestartedLabel_haveRestartedLabel_addsLabel() {
    ...
    V1ObjectMeta want = newObjectMeta().putLabelsItem(RESTARTED_LABEL, RESTARTED_LABEL1);
  ...
  public void addClusterNameLabel_clusteredServer_haveClusterName_addsLabel() {
    ...
    V1ObjectMeta want = newObjectMeta().putLabelsItem(CLUSTERNAME_LABEL, CLUSTER_NAME);
