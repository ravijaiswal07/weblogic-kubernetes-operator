...
public class ConfigMapHelperTest {
  ...
  public void setUp() throws Exception {
    ...
    mementos.add(TestFacade.install());
  }
  ...
  public void loadScripts() throws IOException {
    InMemoryFileSystem.defineFile("scripts/a.b", "1234");
    InMemoryFileSystem.defineFile("scripts/x/c.d", "5678");

    Path p = fileSystem.getPath("scripts");
    Map<String, String> map = ConfigMapHelper.loadContents(p);

    assertThat(map, hasEntry("scripts/a.b", "1234"));
    assertThat(map, hasEntry("scripts/x/c.d", "5678"));
  }
  ...
  static class TestFacade implements ConfigMapHelper.ConfigMapHelperFacade {
    static Memento install() throws NoSuchFieldException {
      return StaticStubSupport.install(ConfigMapHelper.class, "FACADE", new TestFacade());
    }

    @Override
    public boolean containsAll(Map<String, String> actualData, Map<String, String> expectedData) {
      return actualData.keySet().containsAll(expectedData.keySet());
    }
  }
}
